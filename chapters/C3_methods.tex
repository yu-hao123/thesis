...
The problem of estimating $P_{\text{mus}}$, $R$, $E$ within a single cycle is indetermined ($N+2$ variables for $N$ equations),
but the explored algorithms argument is that by introducing constraints on the muscle pressure waveform format and the ranges of possible resistances and compliances, the problem becomes solvable.
...
\hl{TO DO: Description of all attempted maneuvers, R, C won't change
drastically over periods of time, so maneuvers can be performed in spaced intervals}
...
The proposed dual-cycle method evaluates which of the maneuvers described in \Cref{subsec:maneuvers}
are more effective in improving the identifiability of the estimation problem.
...
\hl{TO DO: Details on experimental setup.
 Discuss acquiring data in multiple modes of ventilation (VCV, PCV, PSV).}
...
\hl{TODO: Improve description, This work analyzes a retrospective clinical dataset}~\cite{plens2024}
